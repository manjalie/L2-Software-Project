...
<header class="header">
    <nav class="navbar navbar-expand-lg">
        ...
        @php
            ...
            $l_requests = \App\Lecturer_class_request::with(['lecturer.user','subject'])
            ->whereNull('approved_at')
            ->get();

            $s_requests = \App\student_class_request::with(['student.user','subject'])
            ->whereNull('classroom_added_at')
            ->get();
        @endphp
        ...
        <div class="container-fluid d-flex align-items-center justify-content-between">
            ...
            <div class="right-menu list-inline no-margin-bottom">
                ...
                <div class="list-inline-item dropdown"><a id="navbarDropdownMenuLink1" href="http://example.com" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link messages-toggle">
                        <i class="icon-user-1"></i><span class="badge dashbg-1">{{count($s_requests)}}</span></a>
                    <div aria-labelledby="navbarDropdownMenuLink1" class="dropdown-menu messages">
                        @foreach ($s_requests as $request)
                            <a href="{{route('moderator/student/requests')}}" class="dropdown-item message d-flex align-items-center">
                                <div class="profile">
                                    <img src="{{asset('Profilepic/'.$request->student->user->avatar)}}" alt="..." class="img-fluid">
                                    <div class="status online"></div>
                                </div>
                                <div class="content">
                                    <strong class="d-block">{{$request->student->user->first_name}}</strong>
                                    <span class="d-block">{{$request->subject->name}}</span>
                                    <small class="date d-block">{{$request->created_at}}</small>
                                </div>
                            </a>
                        @endforeach

                                <a href="{{route('moderator/student/requests')}}" class="dropdown-item text-center message">
                                    <strong>See All Requests
                ...
                <div class="list-inline-item dropdown"><a id="navbarDropdownMenuLink2" href="http://example.com" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link tasks-toggle">
                        ...
                        <span class="badge dashbg-3">{{count($l_requests )}}</span></a>
                    <div aria-labelledby="navbarDropdownMenuLink2" class="dropdown-menu tasks-list">

                        @foreach ($l_requests as $request)
                            <a href="{{route('moderator/lecturer/requests')}}" class="dropdown-item message d-flex align-items-center">
                                <div class="profile">
                                    <img src="{{asset('Profilepic/'.$request->lecturer->user->avatar)}}" alt="..." class="img-fluid">
                                    <div class="status online"></div>
                                </div>
                                <div class="content">
                                    <strong class="d-block">{{$request->lecturer->user->first_name}}</strong>
                                    <span class="d-block">{{$request->subject->name}}</span>
                                    <small class="date d-block">{{$request->created_at}}</small>
                                </div>
                            </a>
                        ...
                        <a href="{{route('moderator/lecturer/requests')}}" class="dropdown-item text-center"> <strong>See All Requests <i class="fa fa-angle-right"></i></strong></a>
                    </div>
                </div>
                <!-- Tasks end-->
...
<div class="d-flex align-items-stretch">
    ...
    <nav id="sidebar">
        ...
        <div class="sidebar-header d-flex align-items-center">
            <div class="avatar">
                <a href="{{route('moderator/profile')}}"><img src="{{asset('Profilepic/'.Auth::user()->avatar)}}" alt="..." class="img-fluid rounded-circle"></a>
            </div>
            <div class="title">
        ...
        <ul class="list-unstyled">
            <li class=" {{ Request::path() == 'moderator' ? 'active' : '' }} "><a href="{{route('moderator')}}"> <i class="icon-home"></i>Home </a></li>

       {{---------------------------Lecturers-------------------------------------------------------}}
            <li class ="{{ Request::segment(2) == 'lecturer' ? 'active' : '' }}"><a href="#lecturerdropdown" aria-expanded="false" data-toggle="collapse"> <i class="icon-light-bulb"></i>Lecturer</a>
                <ul id="lecturerdropdown" class="collapse list-unstyled ">
                    <li><a href="{{route('moderator/lecturer/requests')}}">Requests</a></li>
                    <li><a href="{{route('moderator/lecturer/allProfiles')}}">Profile</a></li>
                </ul>
            </li>
        {{-------------------------------------Students------------------------------------------------}}
            <li class ="{{ Request::segment(2) == 'student' ? 'active' : '' }}"><a href="#studentdropdown" aria-expanded="false" data-toggle="collapse"> <i class="icon-user-1"></i>Student</a>
                <ul id="studentdropdown" class="collapse list-unstyled ">
                    <li><a href="{{route('moderator/student/requests')}}">Requests</a></li>
                    <li><a href="{{route('moderator/student/allProfiles')}}">Profile</a></li>
                </ul>
            </li>

        {{-------------------------------------Courses------------------------------------------------}}
            <li class ="{{ Request::segment(2) == 'course' ? 'active' : '' }}"><a href="#coursedropdown" aria-expanded="false" data-toggle="collapse"> <i class="icon-computer"></i>Course</a>
                <ul id="coursedropdown" class="collapse list-unstyled ">
                    <li><a href="{{route('moderator/course/addNew')}}">Add New</a></li>
                    <li><a href="{{route('moderator/course/all')}}">Available</a></li>
                </ul>
            </li>
        {{-------------------------------------Classrooms----------------------------------------------}}
            <li class="{{  Request::segment(2) == 'classroom' ? 'active' : '' }} "><a href="{{route('moderator/classroom')}}">
                    <i class="icon-paper-and-pencil"></i>Classrooms</a>
            </li>
        {{--------------------------------------Payments---------------------------------------------}}
            <li class="{{ Request::path() == 'moderator/payment' ? 'active' : '' }} "><a href="{{route('moderator/payment')}}">
                    <i class="icon-bill"></i>Payments</a>
            </li>
        </ul>
    </nav>
